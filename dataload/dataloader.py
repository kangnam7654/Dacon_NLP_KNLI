...
def build_dataloader(cfg, df, tokenizer, device, mode):
    """
    ...
    dataset = CustomDataset(cfg=cfg, dataframe_set=df, tokenizer=tokenizer, train_stage=param, device=device)
    dataloader = DataLoader(dataset=dataset, batch_size=cfg[f'{mode}'.upper()]['BATCH_SIZE'], shuffle=param, drop_last=False)
    return dataloader
